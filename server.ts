...
// Global Language Middleware (Unified Backend Resolution - User App Authority Only)
app.use((req: any, res, next) => {
  ...
  if (!rawLang) {
    rawLang = req.body?.userProfile?.preferredLanguage || req.body?.userProfile?.lang || req.body?.userProfile?.language || req.body?.userProfile?.idioma || 
              req.body?.profile?.preferredLanguage || req.body?.profile?.lang || req.body?.profile?.language || req.body?.profile?.idioma;
  }
  
  // Deterministic fallback to 'pt' only - NEVER fallback to browser's Accept-Language header
  const resolvedLang: Language = normalizeLang(rawLang);
...
app.post("/api/tarot/interpret", async (req, res) => {
  ...
  const cardsListStr = Array.isArray(cards)
    ? cards.map((c: any) => (typeof c === 'string' ? c : c?.cardName || c?.name || (c?.positionName ? `${c.positionName}: ${c.cardName || c.name}` : String(c)))).join(", ")
    : (typeof cards === 'string' ? cards : "Cartas selecionadas");
