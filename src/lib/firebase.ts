      ...
      if (supported) {
        ...
        if (app) {
          try {
            ...
            console.warn("[Firebase Analytics] Não foi possível inicializar Analytics:", e);
          }
        }
      }
    } catch (err) {
      // Ignore sandbox/iframe security errors
    }
  }
...
// Ensure first connection is validated on startup safely without blocking
